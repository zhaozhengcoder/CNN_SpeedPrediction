...
# 把数据换成一天时间的点
def select_col(orig_data,select_day):
    vol_col=[]
    speed_col=[]
...
def build_newdataframe(speed_col,vol_col,time_col,name_col):
    df =pd.DataFrame(
        {
            'road':'road',
...
def find_col_index(data,columns_name):
...
if __name__=="__main__":
    ...
    """
    rate = miss_rate(data)
    print (filepath+".csv  rate : ",rate)
    if (miss_rate(data) > float(0.5)):
        print ("skip : ",filepath+".csv")
        continue
    """
    # 9-22
    ...
    for day in select_day:
        ...
        result_df.append(build_newdataframe(speed_col,vol_col,time_col,name_col))

    print (filepath+".csv  rate >>> ")
    for i in result_df:
        miss_rate(i)
        #print ("day ",i,"  ,rate ",dayrate)

    os.mkdir(filepath)
    for df ,day in zip(result_df,select_day):
        df.to_csv('./'+filepath+'/'+filepath+'_'+str(day)+'.csv')
